...
(function(D,at,J,aD){D.images=[];
D.options=D.options||{};
var ay=D.options,a=ay.testConn||true,Y=ay.minKbpsForHighBandwidth||300,N=ay.speedTestUri||"http://foresightjs.appspot.com/speed-test/50K",m=ay.speedTestKB||50,g=ay.speedTestExpireMinutes||30,ag=ay.hiResClassname||"fs-high-resolution",C=ay.lowResClassname||"fs-standard-resolution",h="devicePixelRatio",j="devicePixelRatioRounded",v="bandwidth",P="connType",ap="connTestResult",az="connKbps",an="requestChange",S="defaultSrc",y="highResolutionSrc",af="browserWidth",ak="browserHeight",Z="requestWidth",z="requestHeight",i="width",f="height",aa="widthUnits",H="heightUnits",F="aspectRatio",aB="appliedImageSetItem",A="scale",au="scaleRounded",W="uriTemplate",e="uriFind",s="uriReplace",ar="srcModification",ax="loading",B="complete",x="fsjs",G="auto",l="percent",Q="auto",aj="pixel",q="display",aw="auto",I=true,r=false,am=/url\((?:([a-zA-Z-_0-9{}\?=&\\/.:\s]+)|([a-zA-Z-_0-9{}\?=&\\/.:\s]+)\|([a-zA-Z-_0-9{}\?=&\\/.:\s]+))\)/g,O=/[^\d]+/,ae,ah,av=function(){if(ae){return
}ae=ax;
T();
ae=B;
ac()
},ao=function(aF,aE){if(J.createEvent){var aG=J.createEvent("Event");
aG.initEvent("foresight-"+aF,I,I);
aE.dispatchEvent(aG)
}},T=function(){var aE,aF,aH;
for(aE=0;
aE<J.images.length;
aE++){aF=J.images[aE];
if(aF.ignore){continue
}if(!aF.initalized){aF.initalized=I;
ao("imageInitStart",aF);
aF[S]=ai(aF,"src");
aF[aa]=ai(aF,i,I);
aF[H]=ai(aF,f,I);
var aG=ai(aF,"aspect-ratio",r);
aF[F]=aG===G?aG:(!isNaN(aG)&&aG!==null?parseFloat(aG):0);
if(!aF[S]){aF.ignore=I;
continue
}aF[y]=ai(aF,"high-resolution-src");
aF.orgClassName=(aF.className?aF.className:"");
aF.onerror=d;
ao("imageInitEnd",aF);
D.images.push(aF)
}aF.imageSetText=w(aF,"font-family","fontFamily");
aF.imageSet=[];
if(aF.imageSetText.length>1){c(aF,aF.imageSetText.split("image-set(")[1])
}}},M=function(aH){if(aH==""||typeof aH==undefined){return
}var aF,aG,aK;
var aJ=J.getElementById(aH);
var aE=aJ.getElementsByTagName("img");
for(aF=0;
aF<aE.length;
aF++){aG=aE[aF];
if(aG.ignore){continue
}aG.initalized=I;
ao("imageInitStart",aG);
aG[S]=ai(aG,"src");
aG[aa]=ai(aG,i,I);
aG[H]=ai(aG,f,I);
var aI=ai(aG,"aspect-ratio",r);
aG[F]=aI===G?aI:(!isNaN(aI)&&aI!==null?parseFloat(aI):0);
if(!aG[S]){aG.ignore=I;
continue
}aG[y]=ai(aG,"high-resolution-src");
aG.orgClassName=(aG.className?aG.className:"");
aG.onerror=d;
ao("imageInitEnd",aG);
D.images.push(aG);
aG.imageSetText=w(aG,"font-family","fontFamily");
aG.imageSet=[];
if(aG.imageSetText.length>1){c(aG,aG.imageSetText.split("image-set(")[1])
}}},c=function(aE,aI){var aJ,aG=aI!==undefined&&aI!==null?aI.split(","):[],aH,aF;
for(aJ=0;
aJ<aG.length;
aJ++){aH={text:aG[aJ],weight:0};
if(aH.text.indexOf(" 1.5x")>-1){aH.weight++;
aH[A]=1.5
}else{if(aH.text.indexOf(" 2x")>-1){aH[A]=2
}else{if(aH.text.indexOf(" 1x")>-1){aH[A]=1
}}}if(aH.text.indexOf(" high-bandwidth")>-1){aH[v]="high"
}else{if(aH.text.indexOf(" low-bandwidth")>-1){aH[v]="low"
}}while(aF=am.exec(aH.text)){if(aF[1]!=null&&aF[1]!==""){aH[W]=aF[1];
aH.weight++
}else{if(aF[2]!=null&&aF[2]!==""){aH[e]=aF[2];
aH[s]=aF[3];
aH.weight++
}}}if(aH[A]&&aH[v]){aH.weight+=2
}else{if(aH[A]||aH[v]){aH.weight++
}}aE.imageSet.push(aH)
}aE.imageSet.sort(k)
},k=function(aF,aE){if(aF.weight<aE.weight){return 1
}if(aF.weight>aE.weight){return -1
}return 0
},ai=function(aF,aG,aE,aH){aH=aF.getAttribute("data-"+aG);
if(aE&&aH!==null){if(!isNaN(aH)){return parseInt(aH,10)
}return 0
}return aH
},ac=function(){if(!(ah===B&&ae===B)){return
}var aG,aJ=D.images.length,aI,aH,aL,aK,aF=[],aE;
for(aG=0;
aG<aJ;
aG++){aI=D.images[aG];
if(!V(aI)){continue
}ao("imageRebuildStart",aI);
aL=aI.orgClassName.split(" ");
ad(aI);
if(aI.unitType==aj){aK="fs-"+aI[af]+"x"+aI[ak];
aL.push(aK);
if(aF[aK]==undefined){aF[aK]=I;
aF.push("."+aK+"{width:"+((aI[af]>0)?(aI[af]+"px;"):"inherit;")+" height:"+((aI[ak]>0)?(aI[ak]+"px;"):"auto;")+"}")
}}if(aI.style.display!=="inline"){aI.style.display="inline"
}n(aI);
p(aI);
if(D.hiResEnabled&&aI.src!==aI[S]){aL.push(ag)
}else{aL.push(C)
}aL.push("fs-"+aI[ar]);
aI.className=aL.join(" ");
ao("imageRebuildEnd",aI)
}if(aF.length){ab(aF)
}if(D.updateComplete){D.updateComplete()
}b=aA()
},p=function(aG){var aE,aF;
if(aG[aB][A]>1&&aG[aB][v]==="high"){aE=aG[af]===undefined?aw:Math.round(aG[af]*aG[aB][A]);
aF=aG[ak]===undefined?aw:Math.round(aG[ak]*aG[aB][A]);
D.hiResEnabled=I
}else{aE=aG[af]===undefined?aw:aG[af];
aF=aG[ak]===undefined?aw:aG[ak];
D.hiResEnabled=r
}if(!aG[Z]||aE>aG[Z]||aG.activeImageSetText!==aG.imageSetText){aG[Z]=aE;
aG[z]=aF;
if(aG[y]&&D.hiResEnabled){aG.src=aG[y];
aG[ar]="src-hi-res"
}else{aG.src=u(aG)
}aG[an]=I;
aG.activeImageSetText=aG.imageSetText
}else{aG[an]=r
}},u=function(aE){if(aE[aB][W]){aE[ar]="src-uri-template";
return X(aE)
}else{if(aE[aB][e]&&aE[aB][s]){aE[ar]="src-find-replace";
return al(aE)
}}aE[ar]="src-default";
return aE[S]
},n=function(aE){var aH,aF,aG={};
for(aH=0;
aH<aE.imageSet.length;
aH++){aF=aE.imageSet[aH];
if(aF[A]&&aF[v]){if(D[h]==aF[A]&&D[v]===aF[v]){aG=aF;
break
}else{if(Math.round(D[h])==aF[A]&&D[v]===aF[v]){aG=aF;
break
}}}else{if(aF[A]){if(D[h]==aF[A]){aG=aF;
break
}else{if(Math.round(D[h])==aF[A]){aG=aF;
break
}}}else{if(aF[v]){if(D[v]===aF[v]){aG=aF;
break
}}else{aG=aF
}}}}if(!aG[A]){aG[A]=D[h]
}if(!aG[v]){aG[v]=D[v]
}aG[au]=Math.round(aG[A]);
aE[aB]=aG
},V=function(aF,aE){aE=aF.parentNode;
if(aE==null){return r
}if(aE.clientWidth){return I
}if(w(aE,"display")==="inline"){return V(aE)
}return r
},ad=function(aF,aE){if(!aF.unitType){var aG=aF.style[q];
aF.style[q]="none";
aE=w(aF,i);
aF.style[q]=aG;
if((aF[F]&&aE==="auto")||aE.indexOf("%")>0){aF.unitType=l
}else{aF.unitType=aj;
if(aF[F]&&aF[F]!==G){if(aF[H]){aF[af]=Math.round(aF[HEIGHT_UNTIS]/aF[F]);
aF[ak]=aF[H]
}else{aF[af]=aF[aa]||aE.replace(O,"");
aF[ak]=Math.round(aF[af]/aF[F])
}}else{aF[af]=aF[aa];
aF[ak]=aF[H]
}}}if(aF.unitType===l||aF[F]){aF.computedWidth=aq(aF);
aF[af]=aF.computedWidth;
if(aF[F]!=G){aF[ak]=Math.round(aF[af]/aF[F])
}else{if(aF[H]){aF[ak]=Math.round(aF[H]*(aF.computedWidth/aF[aa]))
}}if(aF[ak]&&aD.appVersion.indexOf("MSIE")>-1){aF.style.height=aF[ak]+"px"
}}},aq=function(aF){if(aF.offsetWidth!==0){return aF.offsetWidth
}else{var aH,aG,aE={},aI={position:"absolute",visibility:"hidden",display:"block"};
for(aG in aI){aE[aG]=aF.style[aG];
aF.style[aG]=aI[aG]
}aH=aF.offsetWidth;
for(aG in aI){aF.style[aG]=aE[aG]
}return aH
}},w=function(aF,aE,aG){if(!aG){aG=aE
}return aF.currentStyle?aF.currentStyle[aG]:J.defaultView.getComputedStyle(aF,null).getPropertyValue(aE)
},t,ab=function(aE,aF){if(!t){t=J.createElement("style");
t.setAttribute("type","text/css")
}aF=aE.join("");
try{t.innerHTML=aF
}catch(aG){t.styleSheet.cssText=aF
}if(t.parentElement==null){J.getElementsByTagName("head")[0].appendChild(t)
}},X=function(aF){var aE,aH=["src","protocol","host","port","directory","file","filename","ext","query",Z,z,A,au],aG=aF[aB][W];
aF.uri=aC(aF[S]);
aF.uri.src=aF[S];
aF.uri[Z]=aF[Z];
aF.uri[z]=aF[z];
...
for(aE=0;
aE<aH.length;
aE++){aG=aG.replace("{"+aH[aE]+"}",aF.uri[aH[aE]])
}return aG
},aC=function(aJ){var aI={key:["source","protocol","authority","userInfo","user","password","host","port","relative","path","directory","file","query","anchor"],q:{name:"queryKey",parser:/(?:^|&)([^&=]*)=?([^&]*)/g},parser:/^(?:(?![^:@]+:[^:@\/]*@)([^:\/?#.]+):)?(?:\/\/)?((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/},aE=aI.parser.exec(aJ),aH={},aF=14;
while(aF--){aH[aI.key[aF]]=aE[aF]||""
}aH[aI.q.name]={};
aH[aI.key[12]].replace(aI.q.parser,function(aL,aK,aM){if(aK){aH[aI.q.name][aK]=aM
}});
var aG=aH.file.split(".");
aH.filename=aG[0];
aH.ext=(aG.length>1?aG[aG.length-1]:"");
return aH
},al=function(aE){var aH=aE[aB][e].replace("{browserWidth}",aE[aa]).replace("{browserHeight}",aE[H]);
var aG,aF=aE[S].replace(aH,aE[aB][s]),aI=[Z,z,A,au];
aE[A]=aE[aB][A];
aE[au]=aE[aB][au];
for(aG=0;
aG<aI.length;
aG++){aF=aF.replace("{"+aI[aG]+"}",aE[aI[aG]])
}return aF
},d=function(aE){aE=this;
aE.className=aE.className.replace(ag,C);
aE[ar]="response-error";
if(aE.hasError||aE.src===aE[S]){return
}aE.hasError=I;
aE.src=aE[S]
},E=function(){if(ah){return
}if(ay.forcedBandwidth){D[v]=ay.forcedBandwidth;
D[ap]="forced";
ah=B;
return
}if(D[h]===1){D[ap]="skip";
ah=B;
return
}var aG=aD.connection||{type:"unknown"};
var aJ=aG.type==3||aG.type==4||/^[23]g$/.test(aG.type);
D[P]=aG.type;
if(aJ){D[ap]="connTypeSlow";
ah=B;
return
}try{var aE=JSON.parse(localStorage.getItem(x));
if(aE!==null){if((new Date()).getTime()<aE.exp){D[v]=aE.bw;
D[az]=aE.kbps;
D[ap]="localStorage";
ah=B;
return
}}}catch(aL){}var aI=J.createElement("img"),aF,aK,aH;
aI.onload=function(){aF=(new Date()).getTime();
var aM=(aF-aK)/1000;
aM=(aM>1?aM:1);
D[az]=((m*1024*8)/aM)/1024;
D[v]=(D[az]>=Y?"high":"low");
o("networkSuccess")
};
aI.onerror=function(){o("networkError",5)
};
aI.onabort=function(){o("networkAbort",5)
};
aK=(new Date()).getTime();
ah=ax;
if(J.location.protocol==="https:"){N=N.replace("http:","https:")
}aI.src=N+"?r="+Math.random();
aH=(((m*8)/Y)*1000)+350;
setTimeout(function(){o("networkSlow")
},aH)
},o=function(aH,aF){if(ah===B){return
}ah=B;
D[ap]=aH;
try{if(!aF){aF=g
}var aE={kbps:D[az],bw:D[v],exp:(new Date()).getTime()+(aF*60000)};
localStorage.setItem(x,JSON.stringify(aE))
}catch(aG){}ac()
},K=function(){if(at.addEventListener){at.addEventListener("resize",U,r)
}else{if(at.attachEvent){at.attachEvent("onresize",U)
}}},b=0,U=function(){if(b!==aA()){D.reload()
}},aA=function(){return J.documentElement.clientWidth||J.body&&J.body.clientWidth||1024
},R,L=function(){if(ae!==B||ah!==B){return
}T();
ac()
};
D.resolve=function(aE,aF){aF.imageSet=[];
c(aF,aE);
n(aF);
p(aF);
aF.src=u(aF)
};
D.reload=function(){at.clearTimeout(R);
R=at.setTimeout(L,250)
};
D.ready=function(){if(!J.body){return at.setTimeout(D.ready,1)
}av()
};
D.async=function(aE){if(aE==""||typeof aE==undefined){return
}ae=ax;
M(aE);
ae=B;
ac()
};
if(J.readyState===B){setTimeout(D.ready,1)
}else{if(J.addEventListener){J.addEventListener("DOMContentLoaded",D.ready,r);
at.addEventListener("load",D.ready,r)
}else{if(J.attachEvent){J.attachEvent("onreadystatechange",D.ready);
at.attachEvent("onload",D.ready)
}}}D[h]=at[h]?at[h]:1;
if(ay.forcedPixelRatio){D[h]=ay.forcedPixelRatio
}D[j]=Math.round(D[h]);
E();
K()
...
function k(r,o,q){var p=q.type;
q.type=o;
h.event.dispatch.call(r,q);
q.type=p
...
h.event.special.swipe={scrollSupressionThreshold:30,durationThreshold:1000,horizontalDistanceThreshold:30,verticalDistanceThreshold:75,start:function(o){var p=o.originalEvent.touches?o.originalEvent.touches[0]:o;
return{time:(new Date()).getTime(),coords:[p.pageX,p.pageY],origin:h(o.target)}
},stop:function(o){var p=o.originalEvent.touches?o.originalEvent.touches[0]:o;
return{time:(new Date()).getTime(),coords:[p.pageX,p.pageY]}
...
k(o,"swipe",h.Event("swipe",{target:q,swipestart:s,swipestop:p}));
k(o,r,h.Event(r,{target:q,swipestart:s,swipestop:p}));
return true
}return false
},setup:function(){var o=this,p=h(o);
p.bind(n,function(s){var q,v=h.event.special.swipe.start(s),r=s.target,u=false;
function t(w){if(!v){return
}q=h.event.special.swipe.stop(w);
if(!u){u=h.event.special.swipe.handleSwipe(v,q,o,r)
}if(Math.abs(v.coords[0]-q.coords[0])>h.event.special.swipe.scrollSupressionThreshold){w.preventDefault()
}}p.bind(g,t).one(l,function(){u=true;
p.unbind(g,t)
})
})
},teardown:function(){h(this).unbind(n).unbind(g).unbind(l)
}};
h.each({scrollstop:"scrollstart",taphold:"tap",swipeleft:"swipe",swiperight:"swipe"},function(p,o){h.event.special[p]={setup:function(){h(this).bind(o,h.noop)
...
$j(".suchbox, .suchBoxBottom").live("focus",function(){if($j(this).val().indexOf($j(".suchbox").attr("data-value"))!=-1){$j(this).val("")
}});
$j(".suchbox").live("blur",function(){if($j(this).val().trim()==""){$j(this).val($j(".suchbox").attr("data-value"))
}});
$j(".suchBoxBottom").live("blur",function(){if($j(this).val().trim()==""){$j(this).val($j(".suchbox").attr("data-value"))
}});
$j(".suchButton").live("click",function(){if($j(".suchbox").val()==$j(".suchbox").attr("data-value")){return false
}});
$j(".suchButtonBottom").live("click",function(){if($j(".suchBoxBottom").val()==$j(".suchbox").attr("data-value")){return false
}});
var mouseX;
var mouseY;
...
if(isTouchDevice()&&!iPad&&!iPhone){$j("header #topnavi .tnav a.tn").css("font-size","16px")
...
$j("#sendContainer textarea").live("focus",function(){if($j(this).val().indexOf("[Tragen Sie hier")>-1){$j(this).val("")
}});
...
$j(".callback, .fcallback, .contactLink").unbind("click").click(function(){openIframeShadowBox(860,580,"/callback.html",$j(this).attr("data-title"));
return false
});
...
$j("#productListPagingBottom .more").unbind("click").click(function(){alert("productListPagingBottom");
...
}$j(".contactLayerContainer, .contactLayer").mouseover(function(){$j(".contactLayer").show()
});
$j(".contactLayerContainer, .contactLayer").mouseout(function(){$j(".contactLayer").hide()
})
...
}}function resizeSkyScraper(){var a=window.innerWidth||document.documentElement.clientWidth;
if(!iPad&&!touchDevice){if(a>1180){offset=(window.innerWidth-1180)/2;
$j("#main").css("margin","16px 0px");
$j("#skyScraper").fadeIn();
$j("#skyScraperContainer").css("width","1160px")
}else{if(a<990){$j("#main").css("margin","16px auto");
$j("#skyScraper").fadeOut();
$j("#skyScraperContainer").css("width","990px")
}else{if(a<1180){$j("#main").css("margin","16px auto");
$j("#skyScraper").fadeOut();
breite=a-20;
$j("#skyScraperContainer").css("width",breite+"px")
}}}if(a>1050&&a<1180){$j(".callback").fadeIn()
}else{if(a>1180&&a<1230){$j(".callback").fadeOut()
}else{if(a>1230){$j(".callback").fadeIn()
}else{if(a<990){$j(".callback").fadeOut()
}else{if(a<1050){$j(".callback").fadeOut()
}}}}}}}function hideCallback(){var a=window.innerWidth||document.documentElement.clientWidth;
if(a>1050){$j(".callback").fadeIn()
}else{if(a<990){$j(".callback").fadeOut()
}else{if(a<1050){$j(".callback").fadeOut()
...
$j("#queryStringBottom").autocomplete(baseUrl+"/ajax_suggestSearch.html",{width:260,minChars:2,selectFirst:false});
...
$j("#sendContainer").fadeOut("middle")
})
...
}function copyInMinibasket(){$j(".basketSum").html($j("#intasche .quantity").html())
...
var b="";
if($j(".outfitComplete .outfitCompleteHead.ads span.outfitData").length>0){a=$j(".outfitComplete .outfitCompleteHead.ads span.outfitData").attr("data-outfit-bundle");
b=$j(".outfitComplete .outfitCompleteHead.ads span.outfitData").attr("data-query")
}$j("#imgCarousel").hide();
$j.ajax({type:"GET",url:baseUrl+"/ajax_changeDVModels.html",data:{bid:bid,outfitBid:a,originalQueryString:b,pid:$j(this).attr("data-value"),size:infoSelects.find(".chosenItem span").text(),isFwShop:isFwShop()},dataType:"html",success:function(c){foresight.reload();
$j("#imgCarousel").show();
...
$j(this).addClass("aktiv");
if(isFwShop()){infoSelects.find(".articleNo").html($j(this).attr("alt"))
}else{infoSelects.find(".articleNo").html($j(this).attr("alt")+" "+$j(this).attr("data-articleno-promo"))
}infoSelects.find(".articleNo").parent().show();
...
}function selectDefaultModel(){var a=$j("#product .infoSelects .borderBox .chooseModel li:not(.inactive):first");
...
}function callSizeUpdate(c,a){infoSelects=c.closest(".infoSelects");
mySize="";
...
d=$j(".outfitComplete .outfitCompleteHead.ads span.outfitData").attr("data-query")
}$j.ajax({type:"GET",url:baseUrl+"/ajax_changeDVSizes.html",data:{bid:bid,outfitBid:b,originalQueryString:d,pid:a,size:mySize,isFwShop:isFwShop()},dataType:"html",success:function(f){foresight.reload();
showAvailString(infoSelects);
setSizeUpdateEvent();
if(bid.length>0){if($j("#imgCarousel li").size()>5){$j("#imgCarousel").jcarousel({vertical:true,scroll:5,itemLoadCallback:imgCarousel_itemLoadCallback})
}$j("#imgCarouselContainer .jcarousel-prev").html('<img src="'+rcContextPath+'/images/app/common/bttn/bttn_car_up.png">');
...
}}}var e=$j(".infoSelects .selectPromoItemImg li.aktiv");
if(e.length>0){changeImagesNew($j("#"+e.attr("id").replace("p_","")))
}if(upcload){$j("#showUpcload").show()
}else{$j("#showSizeChart").show()
}},error:function(){}})
...
}else{if(imgSrc.indexOf("formatp")>0){actImg=imgSrc.replace("formatp","formatiI")
}else{if(imgSrc.indexOf("formatL")>0){actImg=imgSrc.replace("formatL","formatI")
}else{if(imgSrc.indexOf("formatl")>0){actImg=imgSrc.replace("formatl","formatI")
}}}}actZoomImg=a.attr("label");
$j("#grossbild").attr("src",actImg);
if(a.attr("data-zoom")!=""){$j("#grossbild").attr("data-zoom",a.attr("data-zoom"))
}}}function changeOutfitImage(a,b,c){if(a.closest(".productContainerOutfits").size()>0){$j(".promoItemPic img").attr("src","http://images.albamoda.com/is/image/AlbaModa/albamoda_formatK/"+b+"/product.jpg");
if(c.attr("data-zoom")!=""){$j(".promoItemPic .mainPic").attr("data-zoom",c.attr("data-zoom"))
}}else{a.closest(".articleItem").find(".articleImg").attr("src","http://images.albamoda.com/is/image/AlbaModa/albamoda_formatP/"+b+"/product.jpg")
...
$j.get(baseUrl+"/ajaxRefreshQuickViewBasket.html")
}})
...
*/
  ...
  if(catalogi(".chosenModel").attr("data-articleno-promo")){
    var articul = catalogi(".chosenModel").attr("data-articleno-promo");
  }else if(catalogi('#modelSelector .aktiv').attr('data-articleno-promo')){
    var articul = catalogi('#modelSelector .aktiv').attr('data-articleno-promo');
  }else{
    var articul = catalogi("#articleNumberWithPromotionCode").val()
  }
  
  if(catalogi('#quickLookHeader b').is('b')){
    var name  = catalogi('#quickLookHeader b').attr('origin');
  }else{
    var name  = catalogi('h1[itemprop="name"]').attr('origin');
  }
  //catalogi('#quickLookHeader b')
  var price = varprice.replace(',','.').replace('EUR','').trim();

  var param = [];

  if(catalogi('#modelSelector .aktiv').is('li')){
    param.push(catalogi('#modelSelector .aktiv').attr('origin'));
  }else if(catalogi('#modelSelector em').is('em')){
  	param.push(catalogi('#modelSelector em').attr('origin').replace(articul,'').trim());
  }

  if(catalogi(".chosenModel").is('li')){
    param.push(catalogi('.chooseModel li.chosenModel').attr('origin').trim());
  }

  if(catalogi(".chosenItem").is('li')){
    param.push(catalogi(".chosenItem").text().trim());
  }
  ...
  catalogi.basket.add({
    ...
    count: 1,
    img: catalogi("#grossbild").attr("src")
  });
...
}location.href=baseUrl+"/premium.html?vid="+vid+"&bid="+$j("#bid").val()
}})
...
$j(document).on("swipeleft",".jcarousel-container",function(b){a.buttonNext.click()
});
$j(document).on("swiperight",".jcarousel-container",function(b){a.buttonPrev.click()
});
...
function initZoom(){foresight.reload();
$j("#zoomCarousel").jcarousel({vertical:true,scroll:4,itemLoadCallback:zoomCarousel_itemLoadCallback});
$j("#zoomCarouselBox .jcarousel-prev").html('<img src="'+rcContextPath+'/images/app/common/bttn/bttn_car_up.png">');
$j("#zoomCarouselBox .jcarousel-next").html('<img src="'+rcContextPath+'/images/app/common/bttn/bttn_car_down.png">');
$j("#zoomCarouselBox").css("visibility","visible");
$j("#DEBUG").hide();
$j("#zoomCarousel img").die("click").live("click",function(){var a=$j(this).attr("data-zoom");
$j("#zoomCarousel img").removeClass("active");
$j(this).addClass("active");
showZoom(a)
})
}function zoomCarousel_itemLoadCallback(a){foresight.reload();
if($j("#zoomCarouselBox .jcarousel-prev-disabled").size()>0){$j("#zoomCarouselBox").css("padding-top","0px")
}else{$j("#zoomCarouselBox").css("padding-top","32px")
...
}function openProductQuickLookInShadowBox(b,a,f,d,c,e){$j.get(baseUrl+"/ajax_productQuickLook.html",{styleId:a,bundleId:b,lmPromo:e,categoryId:c},function(g){Shadowbox.open({content:g,player:"html",title:d,handleOversize:"none",width:"663",modal:true,height:f,options:{onFinish:function(h){foresight.reload();
initQuickLook();
...
$j("#quickviewBasket .loeschen").die().live("click",function(){var d=$j(this).closest("#quickviewBasket").data("removeUrl");
var c=$j(this).parent().attr("data-line-item-id");
$j.get(d,{lineItemId:c});
return false;
$j.ajax({type:"GET",url:baseUrl+"/ajaxDeleteQuickViewBasketLineItem.html",data:"lineItemId="+c,dataType:"html",success:function(e,f){if(e.length>0){$j("#quickviewBasket").html(e)
}if(f=="success"){initQuickviewWarenkorb();
$j("#quickviewBasket").trigger("quickview:removeitem");
if($j("#quickviewBasket li.jcarousel-item-placeholder").size()>0){$j("#quickviewBasket").css("display","none")
...
}function initOutfitLink(){$j(".outfitComplete .outfitCompleteHead.ads a.changeOutfit").live("click",function(){var a=$j(".outfitComplete .outfitCompleteHead.ads span.outfitData");
var h=parseInt(a.attr("data-sum-outfits"));
if(h==1){return false
}var i=parseInt($j(this).attr("data-index"));
var b=a.attr("data-bundle-ids");
var c=a.attr("data-back-bundle");
var d=a.attr("data-is-sale-channel");
var j=a.attr("data-back-category-id");
var f=a.attr("data-article-nr");
var g=a.attr("data-quickview");
var e="";
if(b.split(",").length>i){e=b.split(",")[i]
}$j.ajax({type:"POST",url:baseUrl+"/ajax_changeOutfit.html",data:{bid:e,sid:a.attr("data-style-id"),isSaleChannel:d,backCid:j,articleNumber:f,backBundleId:c,originalQueryString:a.attr("data-query"),quickview:g},dataType:"html",success:function(m){foresight.reload();
var l=i-1;
var k=i+1;
if(l<0){l=h-1
}if(k>=h){k=0
}$j(".outfitComplete .outfitCompleteHead.ads span.outfitIndex").html(i+1);
$j(".outfitComplete .outfitCompleteHead.ads a.prevOutfit").attr("data-index",l);
$j(".outfitComplete .outfitCompleteHead.ads a.nextOutfit").attr("data-index",k);
resizeOutfitComplete()
}});
return false
})
...
}}}function isFwShop(){return $j(".priceContent").hasClass("fwShopPrice")?"true":""
...
function a(f,c){if(f!="-1"&&$j(".dict_"+f).length<1){$j("#main").append("<div class='dictionaryInfo dict_"+f+"'></div>");
$j.ajax({type:"GET",data:{termId:f},url:baseUrl+"/ajax_viewDictionaryTerm.html",dataType:"html",success:function(e){if(e.length>0){$j(".dict_"+f).html(e)
}$j(".dict_"+f).show();
$j(".dict_"+f).css("top",c.pageY+20+"px");
var h=$j("#main").offset();
var g=(c.pageX-h.left);
$j(".dict_"+f).css("left",g+"px");
$j(".dict_"+f).css("z-index","100")
}})
}else{$j(".dict_"+f).show();
$j(".dict_"+f).css("top",c.pageY+20+"px");
var d=$j("#main").offset();
var b=(c.pageX-d.left);
$j(".dict_"+f).css("left",b+"px");
$j(".dict_"+f).css("z-index","100")
...
$j("#suchBoxBottom .suchButtonBottom").live("click",function(){createGoogleEventTracking("Suchbox Suchliste",$j("#queryStringBottom").val(),null)
});
...
Webtrends=(function(){function a(){this._events()
}a.prototype.track=function(b){if((b!=null)&&b.length>0){if($j.inArray("WT.dl",b)){b.push("WT.dl");
b.push("99")
}if(dcsMultiTrack){return dcsMultiTrack.apply(window,b)
}}};
a.prototype._events=function(){var b,c;
b=this;
c=function(h){var g,d,j,f,i,e;
if(h==null){h=""
}if(h!==""){h="-"+h
}i=$j("[data-wt"+h+"]:not([data-wt-init])");
e=[];
for(j=0,f=i.length;
j<f;
j++){g=i[j];
d=$j(g).data("wt");
b.track(d);
e.push($j(g).attr("data-wt-init",true))
}return e
};
c("onload");
$j(document).ajaxSuccess(function(){return c()
});
$j("[data-wt-click]").live("click",function(f){var d;
d=$j(this).data("wt-click");
return b.track(d)
})
};
new a
