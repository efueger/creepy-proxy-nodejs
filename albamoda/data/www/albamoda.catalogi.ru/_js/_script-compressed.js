...
(function(D,ar,J,aC){D.images=[];
D.options=D.options||{};
var ax=D.options,a=ax.testConn||true,X=ax.minKbpsForHighBandwidth||300,M=ax.speedTestUri||"http://foresightjs.appspot.com/speed-test/50K",m=ax.speedTestKB||50,g=ax.speedTestExpireMinutes||30,af=ax.hiResClassname||"fs-high-resolution",C=ax.lowResClassname||"fs-standard-resolution",h="devicePixelRatio",j="devicePixelRatioRounded",v="bandwidth",O="connType",ao="connTestResult",ay="connKbps",al="requestChange",R="defaultSrc",y="highResolutionSrc",ae="browserWidth",aj="browserHeight",Y="requestWidth",z="requestHeight",i="width",f="height",Z="widthUnits",H="heightUnits",F="aspectRatio",aA="appliedImageSetItem",A="scale",at="scaleRounded",V="uriTemplate",e="uriFind",s="uriReplace",aq="srcModification",aw="loading",B="complete",x="fsjs",G="auto",l="percent",P="auto",ai="pixel",q="display",av="auto",I=true,r=false,ak=/url\((?:([a-zA-Z-_0-9{}\?=&\\/.:\s]+)|([a-zA-Z-_0-9{}\?=&\\/.:\s]+)\|([a-zA-Z-_0-9{}\?=&\\/.:\s]+))\)/g,N=/[^\d]+/,ad,ag,au=function(){if(ad){return
}ad=aw;
S();
ad=B;
ab()
},an=function(aE,aD){if(J.createEvent){var aF=J.createEvent("Event");
aF.initEvent("foresight-"+aE,I,I);
aD.dispatchEvent(aF)
}},S=function(){var aD,aE,aG;
for(aD=0;
aD<J.images.length;
aD++){aE=J.images[aD];
if(aE.ignore){continue
}if(!aE.initalized){aE.initalized=I;
an("imageInitStart",aE);
aE[R]=ah(aE,"src");
aE[Z]=ah(aE,i,I);
aE[H]=ah(aE,f,I);
var aF=ah(aE,"aspect-ratio",r);
aE[F]=aF===G?aF:(!isNaN(aF)&&aF!==null?parseFloat(aF):0);
if(!aE[R]||((!aE[Z]||!aE[H])&&!aE[F])){aE.ignore=I;
continue
}aE[y]=ah(aE,"high-resolution-src");
aE.orgClassName=(aE.className?aE.className:"");
aE.onerror=d;
an("imageInitEnd",aE);
D.images.push(aE)
}aE.imageSetText=w(aE,"font-family","fontFamily");
aE.imageSet=[];
if(aE.imageSetText.length>1){c(aE,aE.imageSetText.split("image-set(")[1])
}}},c=function(aD,aH){var aI,aF=aH!==undefined&&aH!==null?aH.split(","):[],aG,aE;
for(aI=0;
aI<aF.length;
aI++){aG={text:aF[aI],weight:0};
if(aG.text.indexOf(" 1.5x")>-1){aG.weight++;
aG[A]=1.5
}else{if(aG.text.indexOf(" 2x")>-1){aG[A]=2
}else{if(aG.text.indexOf(" 1x")>-1){aG[A]=1
}}}if(aG.text.indexOf(" high-bandwidth")>-1){aG[v]="high"
}else{if(aG.text.indexOf(" low-bandwidth")>-1){aG[v]="low"
}}while(aE=ak.exec(aG.text)){if(aE[1]!=null&&aE[1]!==""){aG[V]=aE[1];
aG.weight++
}else{if(aE[2]!=null&&aE[2]!==""){aG[e]=aE[2];
aG[s]=aE[3];
aG.weight++
}}}if(aG[A]&&aG[v]){aG.weight+=2
}else{if(aG[A]||aG[v]){aG.weight++
}}aD.imageSet.push(aG)
}aD.imageSet.sort(k)
},k=function(aE,aD){if(aE.weight<aD.weight){return 1
}if(aE.weight>aD.weight){return -1
}return 0
},ah=function(aE,aF,aD,aG){aG=aE.getAttribute("data-"+aF);
if(aD&&aG!==null){if(!isNaN(aG)){return parseInt(aG,10)
}return 0
}return aG
},ab=function(){if(!(ag===B&&ad===B)){return
}var aF,aI=D.images.length,aH,aG,aK,aJ,aE=[],aD;
for(aF=0;
aF<aI;
aF++){aH=D.images[aF];
if(!U(aH)){continue
}an("imageRebuildStart",aH);
aK=aH.orgClassName.split(" ");
ac(aH);
if(aH.unitType==ai){aJ="fs-"+aH[ae]+"x"+aH[aj];
aK.push(aJ);
if(aE[aJ]==undefined){aE[aJ]=I;
aE.push("."+aJ+"{width:"+aH[ae]+"px;height:"+aH[aj]+"px}")
}}if(aH.style.display!=="inline"){aH.style.display="inline"
}n(aH);
p(aH);
if(D.hiResEnabled&&aH.src!==aH[R]){aK.push(af)
}else{aK.push(C)
}aK.push("fs-"+aH[aq]);
aH.className=aK.join(" ");
an("imageRebuildEnd",aH)
}if(aE.length){aa(aE)
}if(D.updateComplete){D.updateComplete()
}b=az()
},p=function(aF){var aD,aE;
if(aF[aA][A]>1&&aF[aA][v]==="high"){aD=aF[ae]===undefined?av:Math.round(aF[ae]*aF[aA][A]);
aE=aF[aj]===undefined?av:Math.round(aF[aj]*aF[aA][A]);
D.hiResEnabled=I
}else{aD=aF[ae]===undefined?av:aF[ae];
aE=aF[aj]===undefined?av:aF[aj];
D.hiResEnabled=r
}if(!aF[Y]||aD>aF[Y]||aF.activeImageSetText!==aF.imageSetText){aF[Y]=aD;
aF[z]=aE;
if(aF[y]&&D.hiResEnabled){aF.src=aF[y];
aF[aq]="src-hi-res"
}else{aF.src=u(aF)
}aF[al]=I;
aF.activeImageSetText=aF.imageSetText
}else{aF[al]=r
}},u=function(aD){if(aD[aA][V]){aD[aq]="src-uri-template";
return W(aD)
}else{if(aD[aA][e]&&aD[aA][s]){aD[aq]="src-find-replace";
return am(aD)
}}aD[aq]="src-default";
return aD[R]
},n=function(aD){var aG,aE,aF={};
for(aG=0;
aG<aD.imageSet.length;
aG++){aE=aD.imageSet[aG];
if(aE[A]&&aE[v]){if(D[h]==aE[A]&&D[v]===aE[v]){aF=aE;
break
}else{if(Math.round(D[h])==aE[A]&&D[v]===aE[v]){aF=aE;
break
}}}else{if(aE[A]){if(D[h]==aE[A]){aF=aE;
break
}else{if(Math.round(D[h])==aE[A]){aF=aE;
break
}}}else{if(aE[v]){if(D[v]===aE[v]){aF=aE;
break
}}else{aF=aE
}}}}if(!aF[A]){aF[A]=D[h]
}if(!aF[v]){aF[v]=D[v]
}aF[at]=Math.round(aF[A]);
aD[aA]=aF
},U=function(aE,aD){aD=aE.parentNode;
if(aD.clientWidth){return I
}if(w(aD,"display")==="inline"){return U(aD)
}return r
},ac=function(aE,aD){if(!aE.unitType){var aF=aE.style[q];
aE.style[q]="none";
aD=w(aE,i);
aE.style[q]=aF;
if((aE[F]&&aD==="auto")||aD.indexOf("%")>0){aE.unitType=l
}else{aE.unitType=ai;
if(aE[F]&&aE[F]!==G){if(aE[H]){aE[ae]=Math.round(aE[HEIGHT_UNTIS]/aE[F]);
aE[aj]=aE[H]
}else{aE[ae]=aE[Z]||aD.replace(N,"");
aE[aj]=Math.round(aE[ae]/aE[F])
}}else{aE[ae]=aE[Z];
aE[aj]=aE[H]
}}}if(aE.unitType===l||aE[F]){aE.computedWidth=ap(aE);
aE[ae]=aE.computedWidth;
if(aE[F]!=G){aE[aj]=Math.round(aE[ae]/aE[F])
}else{if(aE[H]){aE[aj]=Math.round(aE[H]*(aE.computedWidth/aE[Z]))
}}if(aE[aj]&&aC.appVersion.indexOf("MSIE")>-1){aE.style.height=aE[aj]+"px"
}}},ap=function(aE){if(aE.offsetWidth!==0){return aE.offsetWidth
}else{var aG,aF,aD={},aH={position:"absolute",visibility:"hidden",display:"block"};
for(aF in aH){aD[aF]=aE.style[aF];
aE.style[aF]=aH[aF]
}aG=aE.offsetWidth;
for(aF in aH){aE.style[aF]=aD[aF]
}return aG
}},w=function(aE,aD,aF){if(!aF){aF=aD
}return aE.currentStyle?aE.currentStyle[aF]:J.defaultView.getComputedStyle(aE,null).getPropertyValue(aD)
},t,aa=function(aD,aE){if(!t){t=J.createElement("style");
t.setAttribute("type","text/css")
}aE=aD.join("");
try{t.innerHTML=aE
}catch(aF){t.styleSheet.cssText=aE
}if(t.parentElement==null){J.getElementsByTagName("head")[0].appendChild(t)
}},W=function(aE){var aD,aG=["src","protocol","host","port","directory","file","filename","ext","query",Y,z,A,at],aF=aE[aA][V];
aE.uri=aB(aE[R]);
aE.uri.src=aE[R];
aE.uri[Y]=aE[Y];
aE.uri[z]=aE[z];
...
for(aD=0;
aD<aG.length;
aD++){aF=aF.replace("{"+aG[aD]+"}",aE.uri[aG[aD]])
}return aF
},aB=function(aI){var aH={key:["source","protocol","authority","userInfo","user","password","host","port","relative","path","directory","file","query","anchor"],q:{name:"queryKey",parser:/(?:^|&)([^&=]*)=?([^&]*)/g},parser:/^(?:(?![^:@]+:[^:@\/]*@)([^:\/?#.]+):)?(?:\/\/)?((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/},aD=aH.parser.exec(aI),aG={},aE=14;
while(aE--){aG[aH.key[aE]]=aD[aE]||""
}aG[aH.q.name]={};
aG[aH.key[12]].replace(aH.q.parser,function(aK,aJ,aL){if(aJ){aG[aH.q.name][aJ]=aL
}});
var aF=aG.file.split(".");
aG.filename=aF[0];
aG.ext=(aF.length>1?aF[aF.length-1]:"");
return aG
},am=function(aD){var aG=aD[aA][e].replace("{browserWidth}",aD[Z]).replace("{browserHeight}",aD[H]);
var aF,aE=aD[R].replace(aG,aD[aA][s]),aH=[Y,z,A,at];
for(aF=0;
aF<aH.length;
aF++){aE=aE.replace("{"+aH[aF]+"}",aD[aH[aF]])
}return aE
},d=function(aD){aD=this;
aD.className=aD.className.replace(af,C);
aD[aq]="response-error";
if(aD.hasError||aD.src===aD[R]){return
}aD.hasError=I;
aD.src=aD[R]
},E=function(){if(ag){return
}if(ax.forcedBandwidth){D[v]=ax.forcedBandwidth;
D[ao]="forced";
ag=B;
return
}if(D[h]===1){D[ao]="skip";
ag=B;
return
}var aF=aC.connection||{type:"unknown"};
var aI=aF.type==3||aF.type==4||/^[23]g$/.test(aF.type);
D[O]=aF.type;
if(aI){D[ao]="connTypeSlow";
ag=B;
return
}try{var aD=JSON.parse(localStorage.getItem(x));
if(aD!==null){if((new Date()).getTime()<aD.exp){D[v]=aD.bw;
D[ay]=aD.kbps;
D[ao]="localStorage";
ag=B;
return
}}}catch(aK){}var aH=J.createElement("img"),aE,aJ,aG;
aH.onload=function(){aE=(new Date()).getTime();
var aL=(aE-aJ)/1000;
aL=(aL>1?aL:1);
D[ay]=((m*1024*8)/aL)/1024;
D[v]=(D[ay]>=X?"high":"low");
o("networkSuccess")
};
aH.onerror=function(){o("networkError",5)
};
aH.onabort=function(){o("networkAbort",5)
};
aJ=(new Date()).getTime();
ag=aw;
if(J.location.protocol==="https:"){M=M.replace("http:","https:")
}aH.src=M+"?r="+Math.random();
aG=(((m*8)/X)*1000)+350;
setTimeout(function(){o("networkSlow")
},aG)
},o=function(aG,aE){if(ag===B){return
}ag=B;
D[ao]=aG;
try{if(!aE){aE=g
}var aD={kbps:D[ay],bw:D[v],exp:(new Date()).getTime()+(aE*60000)};
localStorage.setItem(x,JSON.stringify(aD))
}catch(aF){}ab()
},K=function(){if(ar.addEventListener){ar.addEventListener("resize",T,r)
}else{if(ar.attachEvent){ar.attachEvent("onresize",T)
}}},b=0,T=function(){if(b!==az()){D.reload()
}},az=function(){return J.documentElement.clientWidth||J.body&&J.body.clientWidth||1024
},Q,L=function(){if(ad!==B||ag!==B){return
}S();
ab()
};
D.resolve=function(aD,aE){aE.imageSet=[];
c(aE,aD);
n(aE);
p(aE);
aE.src=u(aE)
};
D.reload=function(){ar.clearTimeout(Q);
Q=ar.setTimeout(L,250)
};
D.ready=function(){if(!J.body){return ar.setTimeout(D.ready,1)
}au()
};
if(J.readyState===B){setTimeout(D.ready,1)
}else{if(J.addEventListener){J.addEventListener("DOMContentLoaded",D.ready,r);
ar.addEventListener("load",D.ready,r)
}else{if(J.attachEvent){J.attachEvent("onreadystatechange",D.ready);
ar.attachEvent("onload",D.ready)
}}}D[h]=ar[h]?ar[h]:1;
if(ax.forcedPixelRatio){D[h]=ax.forcedPixelRatio
}D[j]=Math.round(D[h]);
E();
K()
...
imgSrc=imgSrc.split("formatP")[0]+"formatP";
imgSrc+=(typeof promoImg!=="undefined")?"/"+promoImg:"";
imgSrc+=".jpg";
...
$j("#zoomCarousel").jcarousel({vertical:true,scroll:5,itemLoadCallback:zoomCarousel_itemLoadCallback});
...
if($j("#zoomCarouselContainer .jcarousel-prev-disabled").size()>0){$j("#zoomCarouselContainer").css("padding-top","10px")
...
}}function initDetailview(){similarArticlesInfo();
dictionaryInfo();
...
var b=(window.innerWidth-468)/2+"px";
var a=(window.innerHeight-358)/2+"px";
$j("#product #sendContainer").css("top",a);
$j("#product #sendContainer").css("left",b);
...
return false
}$j.ajax({type:"GET",url:baseUrl+"/ajax_productSearch.html",data:{name:$j(".tabNavigation li.showResultForPN").attr("data-name"),productgroup:$j(".tabNavigation li.showResultForPN").attr("data-productgroup"),gender:$j(".tabNavigation li.showResultForPN").attr("data-gender"),styleKey:$j(".tabNavigation li.showResultForPN").attr("data-stylekey")},dataType:"html",success:function(a){foresight.reload();
$j(".tabNavigation li.showResultForPN").addClass("hasContent");
...
return false
}$j.ajax({type:"GET",url:baseUrl+"/ajax_productSearch.html",data:{manufacturer:$j(".tabNavigation li.showResultForM").attr("data-manufacturer"),productgroup:$j(".tabNavigation li.showResultForM").attr("data-productgroup"),gender:$j(".tabNavigation li.showResultForM").attr("data-gender"),styleKey:$j(".tabNavigation li.showResultForM").attr("data-stylekey")},dataType:"html",success:function(a){foresight.reload();
$j(".tabNavigation li.showResultForM").addClass("hasContent");
...
}$j.ajax({type:"GET",url:baseUrl+"/ajax_productSearch.html",data:{manufacturer:$j(".tabNavigation li.showResultForM").attr("data-manufacturer"),productgroup:$j(".tabNavigation li.showResultForPN").attr("data-productgroup"),gender:$j(".tabNavigation li.showResultForM").attr("data-gender"),styleKey:$j(".tabNavigation li.showResultForM").attr("data-stylekey")},dataType:"html",success:function(c){foresight.reload();
$j(".tabNavigation li").removeClass("open");
$j("#product #outfit div.chosen").removeClass("chosen");
...
if($j(".outfitComplete .outfitCompleteHead.ads a.changeOutfit").length>0){initOutfitLink()
...
$j.ajax({type:"GET",url:baseUrl+"/ajax_changeDVModels.html",data:{bid:bid,outfitBid:b,originalQueryString:c,pid:$j(this).attr("data-value"),size:infoSelects.find(".chosenItem span").text(),isFwShop:a},dataType:"html",success:function(d){foresight.reload();
$j("#imgCarousel").show();
showAvailString(infoSelects);
setSizeUpdateEvent();
...
if($j(".priceContent").hasClass("fwShopPrice")){a="true"
}$j.ajax({type:"GET",url:baseUrl+"/ajax_changeDVSizes.html",data:{bid:bid,outfitBid:c,originalQueryString:e,pid:b,size:mySize,isFwShop:a},dataType:"html",success:function(g){foresight.reload();
showAvailString(infoSelects);
setSizeUpdateEvent();
if(bid.length>0){if($j("#imgCarousel li").size()>5){$j("#imgCarousel").jcarousel({vertical:true,scroll:5,itemLoadCallback:imgCarousel_itemLoadCallback})
}$j("#imgCarouselContainer .jcarousel-prev").html('<img src="'+rcContextPath+'/images/app/common/bttn/bttn_car_up.png">');
...
}}}var f=$j(".infoSelects .selectPromoItemImg li.aktiv");
if(f.length>0){changeImagesNew($j("#"+f.attr("id").replace("p_","")))
}},error:function(){}})
...
/*
$j.get(baseUrl+"/ajax_addtobasket.html",{vid:vid,bid:$j("#bid").val(),amount:"1",articleNumberWithPromotionCode:$j("#articleNumberWithPromotionCode").val()},function(a){Shadowbox.open({content:a,player:"html",handleOversize:"none",width:"286",height:"260",options:{overlayOpacity:0.3,onFinish:function(b){copyInMinibasket();
$j("#quickviewBasket").load(baseUrl+"/ajaxRefreshQuickViewBasket.html",function(c,e,d){initQuickviewWarenkorb()
})
}}})
});
...
if(c==""){if(b.length>0){
  ...
  catalogi('.articleCheckBox > input[type="checkbox"]:checked').each(function(){
    ...
    if(catalogi(this).parents('.articleInfos').find('#modelSelector .aktiv').is('li')){
      param.push(catalogi(this).parents('.articleInfos').find('#modelSelector .aktiv').attr('origin'));
    }
  
    if(catalogi(this).parents('.articleInfos').find(".chosenModel").is('li')){
      param.push(catalogi(this).parents('.articleInfos').find('.chooseModel li.chosenModel').attr('origin').trim());
    }
  
    if(catalogi(this).parents('.articleInfos').find(".chosenItem").is('li')){
      param.push(catalogi(this).parents('.articleInfos').find(".chosenItem").text().trim());
    }

    products.push({
      catalog:  'AM',
      articul:  articul,
      name:     '[комплект] - '+catalogi(this).parents('.articleInfos').find('p.productName').attr('origin'),
      price:    catalogi(this).parents('.articleInfos').find('.priceReduced').attr('data-price-computer'),
      size:     (param.length > 0) ? param.join(' ') : '0',
      count:    1
    });
  });
...
/*
  ...
  if($j("#bid").length>0){
    $j("#indenwk").load(baseUrl+"/ajax_addoutfittobasket.html",{vid:a,bid:$j("#bid").val()},function(e,g,f){
      if(g=="success"){refreshMinibasket(-1);
      $j("#quickviewBasket").load(baseUrl+"/ajaxRefreshQuickViewBasket.html",function(h,j,i){
        initQuickviewWarenkorb()
      });
      Shadowbox.close()
    }})
  }else{
    $j("#indenwk").load(baseUrl+"/ajax_addoutfittobasket.html",{vid:a},function(e,g,f){
      if(g=="success"){refreshMinibasket(-1);
      $j("#quickviewBasket").load(baseUrl+"/ajaxRefreshQuickViewBasket.html",function(h,j,i){
        initQuickviewWarenkorb();
        initOutfitToBasketButton()
...
}function initOutfitOrder(){dictionaryInfo();
setSizeUpdateEvent();
...
}$j(".outfitArticleList .articleImg").mouseover(function(){if($j(this).attr("src").indexOf("platzhalter")<0){var b=$j(this).offset().left+83+"px";
var a=(window.innerHeight-450)/2+"px";
$j("#zoom").css("top",a);
$j("#zoom").css("left",b);
$j("#zoom").hide();
zoomImg=$j(this).attr("src");
...
}}function getDVRecommendations(a,b){$j.ajax({type:"GET",url:baseUrl+"/reco.html",data:{artnr:a,styleKey:b},dataType:"html",success:function(c){foresight.reload();
$j(".hasRecomm ul").html(c);
if($j(".hasRecomm ul li").length==0){$j(".showReco").hide();
$j(".hasRecomm").hide();
...
}$j.ajax({type:"GET",url:baseUrl+"/ajax_productSearch.html",data:{manufacturer:$j("ul.tabNavigation").attr("data-manufacturer"),productgroup:$j(".tabNavigation").attr("data-productgroup"),gender:$j(".tabNavigation").attr("data-gender"),styleKey:$j(".tabNavigation").attr("data-style-key")},dataType:"html",success:function(d){foresight.reload();
if($j(".resultOfSearchingForM .imgResultOfCarousel li").size()>3){$j(".resultOfSearchingForM .imgResultOfCarousel").jcarousel({vertical:false,scroll:3,wrap:"circular"})
}}});
return false
}}})
...
function similarArticlesInfo(){$j("body").append("<div class='availabilityInfo'></div>");
$j(".soldItems .alternative").live("click",function(a){$j(".similarArticlesInfo").hide();
...
if($j(".sim_"+varExternalKey+" .info").size()>0){$j(".availabilityInfo").html("GrГ¶Гџe vergriffen? Klicken Sie auf Ihre GrГ¶Гџe,<br>um sich Alternativ-Artikel anzeigen zu lassen!");
$j(".availabilityInfo").show();
$j(".availabilityInfo").css("top",a.pageY+20+"px");
$j(".availabilityInfo").css("left",a.pageX-200+"px");
$j(".availabilityInfo").css("z-index","1000")
}else{if($j(".checked_"+varExternalKey).size()>0){thisItem.children("img").removeClass("hideSold");
$j(".availabilityInfo").html("Ihre gewГ¤hlte AusfГјhrung ist<br>leider ausverkauft.");
$j(".availabilityInfo").show();
$j(".availabilityInfo").css("top",a.pageY+20+"px");
$j(".availabilityInfo").css("left",a.pageX-130+"px");
$j(".availabilityInfo").css("z-index","1000")
}}});
$j(".soldItems li.alternative").live("mouseout",function(a){$j(".availabilityInfo").hide()
})
}function showSimilarArticlesInfo(b,h,f,a,c){if(a){splitVar=b.split("_");
artnr=splitVar[0];
size=splitVar[1];
if($j(".sim_"+b).length<1){$j("#main").append("<div class='similarArticlesInfo sim_"+b+"'></div>");
$j.ajax({type:"GET",data:{articleNumber:artnr,size:size,categoryExternalKey:h},url:baseUrl+"/simularArticle.html",dataType:"html",success:function(e){if(e.length>0){$j(".sim_"+b).html(e);
if($j(".sim_"+b+" .info").size()>0){$j(".availabilityInfo").html("GrГ¶Гџe vergriffen? Klicken Sie auf Ihre GrГ¶Гџe,<br>um sich Alternativ-Artikel anzeigen zu lassen!");
$j(".availabilityInfo").show();
$j(".availabilityInfo").css("top",f.pageY+20+"px");
$j(".availabilityInfo").css("left",f.pageX-200+"px");
...
$j(".availabilityInfo").show();
$j(".availabilityInfo").css("top",f.pageY+20+"px");
$j(".availabilityInfo").css("left",f.pageX-130+"px");
$j(".availabilityInfo").css("z-index","990")
}$j(".availabilityInfo").addClass("checked_"+b)
}})
...
offsetX=550;
offsetY=230;
if($j(".sim_"+b+" .info").size()>0){$j(".sim_"+b).show();
$j(".sim_"+b).css("top",f.pageY-offsetY+"px");
var g=$j("#main").offset();
var d=(f.pageX-g.left);
$j(".sim_"+b).css("left",d-offsetX+"px");
$j(".sim_"+b).css("z-index","100")
