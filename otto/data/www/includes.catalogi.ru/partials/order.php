...
?>
      <?php echo '<tr class="'.$order["id"].'"><td class="flex">'.$order["catalog"].'</td><td class="flex"><img src="'.$order["img"].'" width="50px"></td><td class="flex">'.$order["articul"].'</td><td class="flex">'.$order["size"].'</td><td class="price flex">'.$order["price"].'</td><td>'.$order["naimenovanie"].'</td><td class="flex">'.$order["date"].'</td><td class="quan flex">'.$order["quan"].'</td><td class="last"><a class="delete remove" href="" title="">Удалить артикул</a></td></tr>';?>
<?php endforeach ?>
    </table>  
  </div>
